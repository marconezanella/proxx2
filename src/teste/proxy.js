...
import fs from "fs";
import * as testeDestinoVendas from "./testeDestino";
...
function testarDestino(req, res) {
  ...
  if (destino && destino.redirect) {
    ...
    testeDestinoVendas.verPaginaDeVendas(req, (confirma, error) => {
...
if (fs.existsSync(uri + "/privkey.pem")) {
  //add certificado
  var privateKey = fs.readFileSync(uri + "/privkey.pem", "utf8");
  var certificate = fs.readFileSync(uri + "/fullchain.pem", "utf8");
...
if (fs.existsSync(uri + "/privkey.pem")) httpsServer.listen(443);
// console.log("online");
